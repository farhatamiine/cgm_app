...
class OfflineCacheService {
  ...
  Future<void> _enqueue(String type, Map<String, dynamic> data) async {
    ...
    queue.add({'type': type, 'data': data, 'ts': DateTime.now().toIso8601String()});
  ...
  Future<void> logBolus({
    ...
    final data = {
      ...
      if (glucoseAtInjection != null) 'glucose_at_injection': glucoseAtInjection,
  ...
  Future<void> logBasal({
    ...
    bool savedToApi = false;
    try {
      await _api.logBasal(units: units, insulin: insulin, time: time, notes: notes);
      savedToApi = true;
    } catch (_) {
  ...
  Future<void> logHypo({
    ...
    final data = {
      ...
      if (treatedWith != null) 'treated_with': treatedWith,
      if (notes != null && notes.isNotEmpty) 'notes': notes,
    };
  ...
  double getTodayTotalBolus() {
    return getTodayBoluses().fold(0, (sum, e) => sum + ((e['units'] as num?) ?? 0).toDouble());
  }

  double getTodayTotalBasal() {
    return getTodayBasals().fold(0, (sum, e) => sum + ((e['units'] as num?) ?? 0).toDouble());
  }
